...
Expression* Syntax_expression(Parser* parser, Scope* scope, bool isExpressionRoot) {
	...
	char* const lineUsed = malloc(lineArr.length);
	memset(lineUsed, 0, lineArr.length);

	const int length = lineArr.length;
	Expression* const line = lineArr.data;
	Expression* const line_end = &line[length];
